# -*- coding: utf-8 -*-
# Koios Prontuario | prontuario/telas/tela_login.py
_APP_NOME       = "Prontuario Medico"
_APP_SUBTITULO  = "Sistema de Prontuario Digital"
_APP_ICON_FILE  = "icon.png"
_APP_ICON_VAZIO = "health_and_safety"
_LOGIN_MSG      = "Volte ao Prontuario Medico para continuar."
_PRIVACY_URL    = "https://sanicolauvix.github.io/prontuario-medico-privacidade/"
"""
Fluxo Android:
...
_ROOT = os.path.dirname(os.path.dirname(_HERE))   # Koios root
...
def criar_tela_login(page: ft.Page, on_login_sucesso) -> ft.Container:
    ...
    # Verifica se o usuario ja aceitou a politica de privacidade
    _ja_aceitou = [False]
    if _PRIVACY_URL:
        try:
            from prontuario.dados.model_prontuario import get_config as _gc_p
            _ja_aceitou[0] = (_gc_p("privacidade_aceita", "") == "1")
        except Exception:
            _ja_aceitou[0] = True   # se DB nao existe ainda, nao bloqueia

    _nao_aceito = _PRIVACY_URL and not _ja_aceitou[0]

    btn_entrar = ft.Container(
        content=ft.Row([
            ft.Icon("account_circle_rounded", size=20,
                    color=DIS if _nao_aceito else TXT),
            ft.Text("Entrar com Google", size=15,
                    color=DIS if _nao_aceito else TXT,
                    weight=ft.FontWeight.W_600),
        ], spacing=10, tight=True, alignment=ft.MainAxisAlignment.CENTER),
        padding=ft.padding.symmetric(horizontal=24, vertical=16),
        border_radius=12, bgcolor=CARD,
        ink=True,   # sempre True -- on_click nao dispara com ink=False no Android
        border=ft.Border(
            top=ft.BorderSide(1, BORDA2), bottom=ft.BorderSide(1, BORDA2),
            left=ft.BorderSide(1, BORDA2), right=ft.BorderSide(1, BORDA2),
        ),
        opacity=0.5 if _nao_aceito else 1.0,
    )

    # Checkbox de aceite da politica de privacidade
    _aceite_ref = [_ja_aceitou[0]]
    _icone_aceite = ft.Icon(
        "check_box_rounded" if _aceite_ref[0] else "check_box_outline_blank_rounded",
        size=18,
        color=AZUL if _aceite_ref[0] else DIS,
    )

    def _toggle_aceite(e=None):
        _aceite_ref[0] = not _aceite_ref[0]
        _icone_aceite.name  = "check_box_rounded" if _aceite_ref[0] else "check_box_outline_blank_rounded"
        _icone_aceite.color = AZUL if _aceite_ref[0] else DIS
        btn_entrar.opacity  = 1.0 if _aceite_ref[0] else 0.5
        for ctrl in btn_entrar.content.controls:
            ctrl.color = TXT if _aceite_ref[0] else DIS
        if _aceite_ref[0]:
            try:
                from prontuario.dados.model_prontuario import set_config as _sc_p
                _sc_p("privacidade_aceita", "1")
            except Exception:
                pass
        try: page.update()
        except Exception: pass

    _link_privacidade = ft.Container(
        content=ft.Text(
        ...
        on_click=lambda e: page.launch_url(_PRIVACY_URL) if _PRIVACY_URL else None,
    )

    _row_aceite = ft.Row(
        [
            ft.Container(
                content=_icone_aceite,
                ink=True, border_radius=4,
                padding=ft.padding.all(2),
                on_click=_toggle_aceite,
            ),
            ft.Text("Li e aceito a ", size=12, color=SEC),
            _link_privacidade,
        ],
        spacing=4, tight=True,
        vertical_alignment=ft.CrossAxisAlignment.CENTER,
    ) if _PRIVACY_URL else ft.Container(height=0)
    ...
    def _on_msg(msg):
        ...
        if t == "status":
            ...
            print("[LOGIN] navegando para hub")
            on_login_sucesso()

    page.pubsub.subscribe(_on_msg)

    def _clicou_entrar(e):
        if _PRIVACY_URL and not _aceite_ref[0]:
            page.snack_bar = ft.SnackBar(
                ft.Text("Aceite a Politica de Privacidade para continuar.", color=TXT),
                bgcolor="#1F2937",
            )
    ...
    def _processar_url_redirect(url: str) -> bool:
        try:
            ...
            print(f"[LOGIN] processar_url: url={url[:80]} is_redirect={is_redir}")
            if not is_redir:
                return False
            codigo = extrair_codigo_da_url(url)
            print(f"[LOGIN] codigo extraido: {bool(codigo)}")
            if not codigo:
                return False
            secrets = _estado.get("secrets")
            print(f"[LOGIN] secrets ok={bool(secrets)} fase={_estado.get('fase')}")
            if not secrets:
                return False
            print("[LOGIN] deep link OK -- trocando codigo")
            _pub({"tipo": "processando"})
            threading.Thread(target=_trocar_codigo, args=(secrets, codigo), daemon=True).start()
            return True
        except Exception as ex:
            print(f"[LOGIN] processar_url_redirect erro: {ex}")
            log.warning("[Login] _processar_url_redirect: %s", ex)
            return False

    def _on_lifecycle(e):
        estado = str(getattr(e, "state", "") or "").lower()
        print(f"[LOGIN] lifecycle: state={estado} fase={_estado.get('fase')}")
        if _estado.get("fase") != "instrucoes":
            return
    ...
    def _on_route(e):
        ...
        print(f"[LOGIN] on_route_change: url={url[:100]} fase={_estado.get('fase')}")
    ...
    def _login_android():
        try:
            ...
            if not ok:
                ...
                return
            print(f"[LOGIN] android url_ok={ok} redirect={secrets.get('_redirect_uri', '?')[:60]}")
            ...
            def _iniciar_servidor():
                try:
                    print("[LOGIN] HTTPServer backup em 127.0.0.1:8080")
                    srv = http.server.HTTPServer(("127.0.0.1", 8080), _OAuthHandler)
                    srv.handle_request()
                    srv.server_close()
                    print(f"[LOGIN] HTTP backup concluido: code={bool(codigo_recebido[0])}")
                    if codigo_recebido[0]:
                        _pub({"tipo": "processando"})
                        threading.Thread(
                            target=_trocar_codigo,
                            args=(secrets, codigo_recebido[0]),
                            daemon=True,
                        ).start()
                except Exception as ex:
                    print(f"[LOGIN] _iniciar_servidor erro: {ex}")
    ...
    def _trocar_codigo(secrets: dict, codigo: str):
        try:
            print(f"[LOGIN] trocando codigo len={len(codigo)}")
    ...
    bloco_central = ft.Column(
        [
            ...
            _row_aceite,
            ft.Container(height=10) if _PRIVACY_URL else ft.Container(height=0),
            btn_entrar,
            ft.Container(height=10),
            painel_instrucoes,
